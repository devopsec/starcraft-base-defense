...
public class InverseKinematics2 : MonoBehaviour {
    ...
    void LateUpdate() 
    {
        if (upperArm != null && forearm != null && hand != null && elbow != null && target != null) {
            ...
            // If player position invalid return
            if (float.IsNaN(upperArm.position.x) || float.IsNaN(cross.x) || float.IsNaN(angle)) {
                return;
            }
